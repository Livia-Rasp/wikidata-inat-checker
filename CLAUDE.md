...
- **Outputs → `output/`:** deliverables you act on — `output/drafts.html` (images); `output/names.html`; `output/links.html` + `output/links-ambiguous.html` + `output/links-auto.qs` + `output/inat-links-conflicts.json` (links); `output/area.html`. Safe to delete wholesale; a re-run regenerates them. (`web/data/taxa.json` is still written but no longer read — the app gets its worklist from `GET /api/findings`; the exporter goes in slice 4.)
