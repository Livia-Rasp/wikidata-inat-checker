...
- **Findings DB → `data/findings.db`: NOT safe to delete.** The image checker records every outcome here instead of a tombstone cache — `open` (photos + a draft), `no_draft` (photos but no P225/family template), `no_photos`, later `done`/`skipped`/`fixed_upstream`/`gone` — so the backlog **accumulates across runs** rather than being overwritten, and `output/drafts.html` + `web/data/taxa.json` render the whole open worklist. Negative outcomes carry `checked_at` and expire after `--recheck-after` days (default 90), because photos and P3151 links keep appearing; settled ones never expire. A failed iNat batch records *nothing*, so it retries. SQLite via `node:sqlite`, WAL + `STRICT`, schema version in `PRAGMA user_version` (`lib/db.js`). Migrating the other three checkers onto it is [docs/findings-db-roadmap.md](docs/findings-db-roadmap.md) slices 6–8.
