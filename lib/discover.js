...
export class DiscoveryError extends Error {
    ...
    constructor(code, message, details = {}) {
        ...
        // A bad scope is the caller's mistake, not a defect: `runMain` prints the message without a
        // stack trace, and the route layer turns it into a 400 rather than a 500.
        this.expected = true;
    }
...
export async function discover({
    ...
    candidateSource, generateDrafts = generateDraftWikitext,
}) {
    ...
    try {
        ...
        // hit the same early slice first. The IUCN branch needs no shuffle — it queries WDQS
        // directly for the whole (already-selective) matching set rather than enumerating by id.
        const source = candidateSource ?? (iucn
            ? fetchWdImagesByIucn(iucn.qid)
            : fetchWdTaxaByInatIds(shuffle(scoped?.ids ?? taxaDb.allInatIds(), seed)));
