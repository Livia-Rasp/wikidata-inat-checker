...
export async function fetchWithRetry(doFetch, retries, label = 'SPARQL') {
    ...
    try {
        ...
        if (retries > 0 && (err.name === 'TimeoutError' || err.name === 'AbortError')) {
            ...
            console.warn(`${label} request timed out, retrying in ${delay / 1000}s...`);
...
export async function sparql(query, retries = 3) {
    ...
    try {
        ...
        if (retries > 0) {
            const delay = sparqlRetryDelay(null, retries);
            console.warn(`SPARQL response failed to parse, retrying in ${delay / 1000}s...`);
            await new Promise(r => setTimeout(r, delay));
 ...
 * @param {string[]} values
 * @param {{ valueProperty: string, absentProperty?: string | null, valueKey: string, sparqlPostFn?: (q: string) => Promise<Record<string, string>[]>, batchSize?: number, iucnQid?: string | null, onBatch?: (done: number, total: number) => void }} opts
 * @returns {AsyncGenerator<{ wdUri: string, qid: string, iucnQid: string | null, [k: string]: string | null }>}
 */
export async function* fetchWdTaxaByValues(values, { valueProperty, absentProperty = null, valueKey, sparqlPostFn = sparqlPost, batchSize = 10000, iucnQid = null, onBatch }) {
 ...
 * @param {{ valueProperty: string, absentProperty?: string | null, valueKey: string, sparqlFn?: (q: string) => Promise<Record<string, string>[]> }} opts
 * @returns {AsyncGenerator<{ wdUri: string, qid: string, iucnQid: string, [k: string]: string }>}
 */
export async function* fetchWdTaxaByIucn(iucnQid, { valueProperty, absentProperty = null, valueKey, sparqlFn = sparqlTSV }) {
