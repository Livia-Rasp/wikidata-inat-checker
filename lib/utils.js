...
/**
 * Sent on every outbound request — Wikidata, iNaturalist, Commons — from a dozen call sites.
 * Wikimedia blocks anonymous clients, so the contact URL is the part that must never be dropped.
 *
 * The version is read rather than written: it used to be the string `1.0.0` hardcoded here, which
 * is a second place a release lives and the one nobody remembers. It would have gone on announcing
 * 1.0.0 indefinitely, and nothing tests a User-Agent, so the drift would be silent.
 * @type {Record<string, string>}
 */
export const HEADERS = {
    'User-Agent': `wikidata-inat-checker/${pkg.version} (https://github.com/Livia-Rasp/wikidata-inat-checker)`
};
 ...
 *
 * `log` defaults to the whole `console` object, not a single function — CLI callers get today's
 * unchanged behaviour, and the server passes its real request-scoped logger (which has the same
 * `.warn`/`.error` shape) so retries show up structured, tagged with the request that triggered
 * them, rather than as plain stdout text nothing captures once the server no longer has a
 * terminal attached.
 ...
 */
export async function fetchWithRetry(doFetch, retries, label = 'SPARQL', log = console) {
    let res;
    try {
        res = await doFetch();
    } catch (err) {
        // AbortSignal.timeout() rejects with a TimeoutError (or AbortError) instead of
        // resolving with a bad status — a hung connection never reaches the status check
        // below, so it needs its own retry path or it kills the whole run uncaught.
        if (retries > 0 && (err.name === 'TimeoutError' || err.name === 'AbortError')) {
            const delay = sparqlRetryDelay(null, retries);
            log.warn(`${label} request timed out, retrying in ${delay / 1000}s...`);
            await new Promise(r => setTimeout(r, delay));
            return fetchWithRetry(doFetch, retries - 1, label, log);
        }
        throw err;
    }
    if (RETRYABLE_STATUS.has(res.status) && retries > 0) {
        const delay = sparqlRetryDelay(res.status, retries);
        log.warn(`${label} HTTP ${res.status}, retrying in ${delay / 1000}s...`);
        await new Promise(r => setTimeout(r, delay));
        return fetchWithRetry(doFetch, retries - 1, label, log);
    }
...
/**
 * @typedef {Record<string, {value: string}>} SparqlBindingRow one row of raw SPARQL JSON results
 *   — as opposed to sparqlTSV/sparqlPost's flattened { [varName]: string }, unbound (OPTIONAL)
 *   variables are simply absent keys here, and every bound one is `{value: string}` to unwrap.
 */
 ...
 * @param {number} [retries]
 * @param {{warn: Function}} [log]
 * @returns {Promise<SparqlBindingRow[]>} SPARQL result bindings
 */
export async function sparql(query, retries = 3, log = console) {
    const res = await fetchWithRetry(
        () => fetch(wbk.sparqlQuery(query), reqInit(SPARQL_TIMEOUT_MS)), retries, 'SPARQL', log);
    const text = await res.text();
    // Some Wikidata string values contain literal C0 control characters (invalid JSON).
    // oxlint-disable-next-line no-control-regex -- intentional: stripping exactly those bytes.
    const cleaned = text.replace(/[\x00-\x1F\x7F]/g, '');
    try {
        return JSON.parse(cleaned).results.bindings;
    } catch (err) {
        // WDQS under load has been seen returning a 200 with a body truncated mid-stream
        // (valid HTTP, invalid JSON) — the same silent-incomplete-response failure mode as its
        // row-undercount behavior, just caught here as a parse error instead. A fresh request
        // usually gets a complete body, so this is worth a retry rather than a hard failure.
        if (retries > 0) {
            const delay = sparqlRetryDelay(null, retries);
            log.warn(`SPARQL response failed to parse, retrying in ${delay / 1000}s...`);
            await new Promise(r => setTimeout(r, delay));
            return sparql(query, retries - 1, log);
        }
        throw err;
    }
}
 ...
 *          concurrency?: number, batchSize?: number, log?: {warn: Function},
 ...
 */
export async function sparqlTSV(query, retries = 3, log = console) {
    ...
    const res = await fetchWithRetry(
        () => fetch(url, reqInit(SPARQL_TIMEOUT_MS,
 ...
 * @returns {Promise<Record<string, string>[]>}
 */
export async function sparqlPost(query, retries = 3, log = console) {
    const res = await fetchWithRetry(() => fetch(SPARQL_ENDPOINT, {
        method: 'POST',
        ...reqInit(SPARQL_TIMEOUT_MS, {
            ...HEADERS,
 ...
 */
export async function cirrusCount(srsearch, retries = 3, log = console) {
    const params = new URLSearchParams({
        action: 'query', list: 'search', srsearch,
        srnamespace: '0', srlimit: '1', srinfo: 'totalhits', srprop: '', format: 'json',
    });
    const res = await fetch(`${WD_API_ENDPOINT}?${params}`, reqInit());
    if (res.status === 429 && retries > 0) {
        log.warn('CirrusSearch HTTP 429, retrying in 30s...');
        await new Promise(r => setTimeout(r, 30000));
        return cirrusCount(srsearch, retries - 1, log);
    }
...
/**
 * Persist the Commons category-existence cache to disk. Best-effort: this is a cross-run
 * performance cache, not correctness state, and `cache/` is not writable inside the deployed
 * container (`read_only: true` root, no volume mounted there) — a discovery run that finds an
 * actionable candidate reaches this on every batch, so before this guard existed, running
 * discovery in that container failed the whole batch (findings and all) the first time it did.
 * Losing the cache there just means re-checking category existence against the live API next
 * time, the same as a first run anywhere else.
 */
export function saveCommonsCatCache() {
    if (!catCache) return;
    try {
        fs.writeFileSync(ensureParentDir(COMMONS_CAT_CACHE_FILE), JSON.stringify(catCache, null, 2), 'utf8');
    } catch { /* read-only filesystem (e.g. the container) — the cache just won't persist */ }
}
 ...
 * @param {{warn: Function}} [log]
 */
export async function checkCommonsCategories(names, log = console) {
    ...
    for (const batch of chunk(todo, 45)) {
        try {
            ...
            const res = await fetch(`${COMMONS_API}?${params}`, reqInit());
            ...
            log.warn(`Commons category check failed: ${e.message}`);
 ...
 */
export async function resolveCommonsCategory(name, maxHops = 3, log = console) {
    const cache = ensureCatCache();
    let cur = name;
    for (let i = 0; i <= maxHops; i++) {
        await checkCommonsCategories([cur], log);
        const v = cache[cur];
 ...
 * `valueProperty` — and, when `absentProperty` is given, lack it — by querying Wikidata *by
 * value* in bounded VALUES POST batches. This sidesteps WDQS's inability to scan large
 ...
 * @param {string[]} values
 * @param {{ valueProperty: string, absentProperty?: string | null, valueKey: string, log?: {warn: Function}, sparqlPostFn?: (q: string) => Promise<Record<string, string>[]>, batchSize?: number, iucnQid?: string | null, onBatch?: (done: number, total: number) => void }} opts
 * @returns {AsyncGenerator<{ wdUri: string, qid: string, iucnQid: string | null, [k: string]: string | null }>}
 */
export async function* fetchWdTaxaByValues(values, {
    valueProperty, absentProperty = null, valueKey, log = console,
    // sparqlPostFn's default closes over `log` (destructured just above, so it's already bound
    // by the time this default runs) — a caller overriding sparqlPostFn gets whatever behaviour
    // it wants; a caller only overriding `log` still gets it threaded through the real sparqlPost.
    sparqlPostFn = (q) => sparqlPost(q, 3, log),
    batchSize = 10000, iucnQid = null, onBatch,
}) {
...
${absentProperty ? `  FILTER NOT EXISTS { ?item wdt:${absentProperty} ?x . }\n` : ''}  OPTIONAL { ?item wdt:P141 ?iucn . }
}`);
        for (const r of rows) {
            if (!r.item) continue;
 ...
 * @param {string[]} names
 * @param {{ sparqlPostFn?: (q: string) => Promise<Record<string, string>[]>, batchSize?: number, iucnQid?: string | null, onBatch?: (done: number, total: number) => void, log?: {warn: Function} }} [opts]
 */
...
/**
 * Wikidata taxa with a P3151 iNat ID matching one of `ids` — no absence filter. Thin wrapper
 * over {@link fetchWdTaxaByValues}; yields `{ wdUri, qid, inatId, iucnQid }`. For the names
 * checker, which wants every already-linked taxon and filters on P1843 itself downstream,
 * rather than a taxon missing some specific property.
 * @param {string[]} ids
 * @param {{ sparqlPostFn?: (q: string) => Promise<Record<string, string>[]>, batchSize?: number, iucnQid?: string | null, onBatch?: (done: number, total: number) => void, log?: {warn: Function} }} [opts]
 */
export function fetchWdTaxaLinkedByInatIds(ids, opts = {}) {
    return fetchWdTaxaByValues(ids, { ...opts, valueProperty: 'P3151', valueKey: 'inatId' });
}

/**
 * Enumerate Wikidata taxa (P31=Q16521) that carry a value on `valueProperty` and a given
 * IUCN status (P141=iucnQid) — and, when `absentProperty` is given, lack it — via one direct
 * SPARQL query. With P141 as the selective constraint the filtered set is small (a few
 * thousand to ~30k rows), so WDQS answers it in seconds — unlike the unfiltered
 * absent-property sets (hundreds of thousands to millions of rows) which it cannot scan, and
 * which is why {@link fetchWdTaxaByValues} inverts the query to enumerate by value instead.
 * Prefer this whenever an IUCN status is specified. Yields `{ wdUri, qid, [valueKey], iucnQid }`.
 * @param {string} iucnQid
 * @param {{ valueProperty: string, absentProperty?: string | null, valueKey: string, log?: {warn: Function}, sparqlFn?: (q: string) => Promise<Record<string, string>[]> }} opts
 * @returns {AsyncGenerator<{ wdUri: string, qid: string, iucnQid: string, [k: string]: string }>}
 */
export async function* fetchWdTaxaByIucn(iucnQid, {
    valueProperty, absentProperty = null, valueKey, log = console,
    sparqlFn = (q) => sparqlTSV(q, 3, log),
}) {
 ...
 * @param {string} iucnQid
 * @param {{ sparqlFn?: (q: string) => Promise<Record<string, string>[]>, log?: {warn: Function} }} [opts]
 */
...
/**
 * {@link fetchWdTaxaByIucn} for the names target: P3151 present, an IUCN status, no absence filter.
 * @param {string} iucnQid
 * @param {{ sparqlFn?: (q: string) => Promise<Record<string, string>[]>, log?: {warn: Function} }} [opts]
 */
export function fetchWdNamesByIucn(iucnQid, opts = {}) {
    return fetchWdTaxaByIucn(iucnQid, { ...opts, valueProperty: 'P3151', valueKey: 'inatId' });
}
...
/**
 * Shared fallback seed for every checker's `--seed` flag, so a from-scratch scan is reproducible
 * for debugging by default (same seed, same order, every run) and diverges only when asked to.
 */
export const DEFAULT_SCAN_SEED = 42;

/**
 * Parse a `--seed` from parsed args, falling back to {@link DEFAULT_SCAN_SEED}. Unlike
 * `parseLimit`, `0` is a legitimate seed, so validity is `Number.isFinite`, not truthiness — a
 * literal `--seed 0` must not silently become the default.
 * @param {Record<string, string | true>} args
 * @returns {number}
 */
export function parseSeed(args) {
    const n = Number.parseInt(/** @type {string} */ (args.seed), 10);
    return Number.isFinite(n) ? n : DEFAULT_SCAN_SEED;
}

/**
 * Deterministic Fisher-Yates shuffle (mulberry32 PRNG). Same seed -> same order every run,
 * so a from-scratch scan is reproducible for debugging even though live rescans will
 * naturally diverge once the cache starts skipping already-collected candidates. Used to
 * de-bias enumeration order against whatever incidental order the underlying source returns
 * (SQLite's row order, WDQS's default order) — without it, a limited scan always sees the
 * same early slice of that order first.
 * @template T
 * @param {T[]} arr
 * @param {number} seed
 * @returns {T[]}
...
/** The reverse of {@link WD_RANK_LABELS} — which QID identifies a given rank, the single source
 *  used everywhere a specific rank (genus, family, ...) needs naming rather than just comparing. */
export const RANK_QID = Object.fromEntries(
    Object.entries(WD_RANK_LABELS).map(([qid, label]) => [label, qid]));
...
export function parseIucnArg(args) {
    ...
    // Throws rather than exits: a library that kills the process cannot be reused anywhere the
    // process is not disposable — the same reason lib/discover.js stopped doing it. `runMain`
    // owns the exit.
    if (iucnArg && !iucnQid) {
        throw new UsageError(`Unknown IUCN status "${iucnArg}".`,
            [`Valid codes: ${Object.keys(IUCN_STATUS_QIDS).join(', ')}`]);
    }
    return { iucnArg, iucnQid };
}

// Measured live against WDQS (250 real species-rank taxa, 2026-08-22): batches of 50 ran at
// ~2.3s/batch (~46ms/item); one batch of 100 ran in 1.28s (~13ms/item) — per-query overhead, not
// per-item transitive-closure cost, dominates at 50, so doubling the batch roughly halves total
// round trips for a modest, sub-linear rise in per-query time. 100 keeps single-batch time well
// under SPARQL_TIMEOUT_MS even if real batches (denser multi-parent taxa, the ones this function
// exists to handle correctly) cost more per item than that sample did.
const ANCESTOR_CHAIN_BATCH = 100;
// P171+ is a transitive closure computed per item, far heavier server-side than a flat value
// lookup, and WDQS has already shown instability under this exact query (v1.1.1's retry
// hardening) — stay well under the Action API's concurrency-of-3 convention.
const ANCESTOR_CHAIN_CONCURRENCY = 2;

/**
 * Fetch the full Wikidata ancestor chain for each item in `items` via wdt:P171+, up to
 * `concurrency` batches of `ANCESTOR_CHAIN_BATCH` items in flight at once — this query is the
 * slowest step in a full links-checker run, so batches overlap rather than running strictly
 * sequentially.
 * Returns a Map<qid, {name, rankQid}[]> with chains in kingdom-first order.
 * @param {{ qid: string }[]} items
 * @param {(query: string) => Promise<SparqlBindingRow[]>} sparqlFn
 ...
 */
export async function fetchWdAncestorChains(items, sparqlFn, qidFromUriFn, chunkFn, opts = {}) {
    const { concurrency = ANCESTOR_CHAIN_CONCURRENCY } = opts;
    const limit = pLimit(concurrency);
    const batches = chunkFn(items, ANCESTOR_CHAIN_BATCH);
    const batchEntries = await Promise.all(batches.map(batch => limit(async () => {
  ...
  OPTIONAL {
    ...
    OPTIONAL { ?ancestor wdt:P171 ?ancestorParent . }
        ...
        for (const b of bindings) {
            ...
            if (b.ancestor) {
                // An ancestor can carry more than one wdt:P171 (Wikidata's taxonomic backbone is
                // not a strict tree), so SPARQL emits one row per parent — collect every candidate
                // rather than letting the last row silently overwrite the others.
                if (!d.ancestors.has(b.ancestor.value)) d.ancestors.set(b.ancestor.value, {
                    name:    b.ancestorName?.value ?? '',
                    rankQid: b.ancestorRank?.value?.split('/').pop() ?? null,
                    parents: [],
                });
                const parent = b.ancestorParent?.value;
                const a = d.ancestors.get(b.ancestor.value);
                if (parent && !a.parents.includes(parent)) a.parents.push(parent);
            }
        }
        /** @type {[string, {name: string, rankQid: string|null}[]][]} */
        const entries = [];
        for (const [itemUri, { directParent, ancestors }] of byItem) {
            const chain = [];
            const seen = new Set();
            let cur = directParent;
            while (cur && ancestors.has(cur) && !seen.has(cur)) {
                seen.add(cur);
                const a = ancestors.get(cur);
                chain.push({ name: a.name, rankQid: a.rankQid });
                // Prefer whichever candidate parent is itself a known ancestor of the item — the
                // one that keeps walking the item's own chain rather than veering onto a parallel
                // classification that dead-ends outside what P171+ returned for this item.
                cur = a.parents.find(p => ancestors.has(p)) ?? a.parents[0] ?? null;
            }
            entries.push([qidFromUriFn(itemUri), chain.reverse()]);
        }
        return entries;
    })));
    const treeMap = new Map();
    for (const entries of batchEntries) for (const entry of entries) treeMap.set(...entry);
    return treeMap;
}
