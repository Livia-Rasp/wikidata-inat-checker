...
import { reqInit, qidFromUri, chunk, fetchEntitiesBatched, IUCN_QID_TO_CODE, checkCommonsCategories, resolveCommonsCategory, saveCommonsCatCache, RANK_QID } from './utils.js';
...
// QIDs from RANK_QID (lib/utils.js), the single source for which QID identifies which rank.
const {
    genus: RANK_GENUS, family: RANK_FAMILY, order: RANK_ORDER, subclass: RANK_SUBCLASS,
    class: RANK_CLASS, superfamily: RANK_SUPERFAMILY, subfamily: RANK_SUBFAMILY,
    tribe: RANK_TRIBE, subtribe: RANK_SUBTRIBE,
} = RANK_QID;
const RANK_LABELS   = {
    [RANK_CLASS]:       'Classis',
...
async function fetchTaxonavTemplates(log = console.log) {
    ...
    log(`Loaded ${lookup.size} Taxonavigation templates from Commons.`);
    return lookup;
}
 ...
 */
export async function fetchEntities(qids, log = console) {
    return fetchEntitiesBatched(qids, { props: 'claims|sitelinks', sitefilter: 'specieswiki', log });
}
...
//
// `cache` is supplied by the caller so it can span a whole run rather than one call: taxa share
// their upper lineage almost entirely, so a second batch that starts from an empty cache re-fetches
// the same kingdom-level ancestors it just walked.
async function buildAncestorCache(itemQids, cache = {}) {
...
/**
 * The pure, network-free half of this module, exported for `test/wikitext.test.js`.
 *
 * These are the functions that decide what a draft actually says — which Taxonavigation template
 * a taxon gets, which ranks are listed by hand, whether {{IUCN}} or a plain maintenance category
 * is emitted, which "Endemic <group> of <place>" titles are worth asking Commons about. They are
 * also the ones a reviewer would want to see covered.
 *
 * They are grouped here rather than exported individually because none of them is part of this
 * module's real interface: `generateDraftWikitext` is. `generateDraftWikitext` reaches for the
 * network through module-level helpers with no injection point (unlike lib/confirm.js and
 * lib/verify.js, which take an API function), so covering this logic through it would mean
 * mocking global fetch. Exporting the pure functions is the cheaper and more honest option.
 *
 * Nothing outside the test suite should import this.
 */
export const __testables = {
    ...
    parseEntity,
    endemicGroupWords,
    endemicTitlesForPlace,
    endemicCandidateTitles,
};

/**
 * State worth keeping for the length of one run rather than one call: the ancestor lineages (taxa
 * overwhelmingly share their upper branches) and the Commons Taxonavigation template map (10–40
 * requests, and the set changes on the order of weeks). Discovery records findings batch by batch,
 * so without this every batch would pay for both again.
 * @returns {{ancestors: Record<string, any>, templates: Map<string, string>|null}}
 */
export function createWikitextContext() {
    return { ancestors: {}, templates: null };
}

/**
 * @param {Record<string, true>} available - wdUri → true for taxa with qualifying iNat photos
 * @param {{context?: ReturnType<typeof createWikitextContext>, log?: (msg: string) => void}} [opts]
 * @returns {Promise<Record<string, string>>} wdUri → Commons category Wikitext draft
 */
export async function generateDraftWikitext(available, opts = {}) {
    const { context = createWikitextContext(), log = console.log } = opts;
    const wdUris = Object.keys(available);
    if (wdUris.length === 0) {
        log('No available items, skipping draft generation.');
        return {};
    }
    log(`Generating Wikitext drafts for ${wdUris.length} items...`);
    ...
    const [cache, templates] = await Promise.all([
        buildAncestorCache(qids, context.ancestors),
        context.templates ?? fetchTaxonavTemplates(log).then(t => (context.templates = t)),
    ]);
