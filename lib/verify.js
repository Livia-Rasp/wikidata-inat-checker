 ...
 *           fetchFn?: (qids: string[]) => Promise<object>, log?: {warn: Function},
 *           onProgress?: (done: number, total: number) => void }} [opts]
 * @returns {Promise<VerifyResult>}
 */
export async function verifyOpenFindings(store, opts = {}) {
    const { kind = 'image', limit, fetchFn, log, onProgress } = opts;
    if (kind === 'link') return verifyLinkFindings(store, { limit, fetchFn, log, onProgress });
    if (kind === 'name') return verifyNameFindings(store, { limit, fetchFn, log, onProgress });
 ...
 * @param {ReturnType<typeof import('./db.js').createFindingsStore>} store
 * @param {{ limit?: number, fetchFn?: (qids: string[]) => Promise<object>, log?: {warn: Function},
 *           onProgress?: (done: number, total: number) => void }} opts
 * @returns {Promise<VerifyResult>}
 */
async function verifyLinkFindings(store, { limit, fetchFn, log, onProgress }) {
    const candidates = [
    ...
    const entities = await fetchEntitiesBatched([...qidsToFetch], { props: 'claims', redirects: 'no', fetchFn, log });
 ...
 * @returns {Promise<VerifyResult>}
 */
async function verifyNameFindings(store, { limit, fetchFn, log, onProgress }) {
    ...
    const entities = await fetchEntitiesBatched(scoped.map(r => r.qid), { props: 'claims', redirects: 'no', fetchFn, log });
