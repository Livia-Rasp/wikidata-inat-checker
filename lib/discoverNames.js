...
import { resolveIucn, resolveTaxonScope, runProgress, runFinishFields } from './discover.js';
...
export async function discoverNames({
    ...
    fetchEntitiesFn = (qids) => fetchEntities(qids, log), fetchInatNamesFn = fetchInatNames,
}) {
    ...
    try {
        ...
        const source = candidateSource ?? (iucn
            ? fetchWdNamesByIucn(iucn.qid, { log })
            : fetchWdTaxaLinkedByInatIds(shuffle(taxaDb.allInatIds(), seed), { log }));
