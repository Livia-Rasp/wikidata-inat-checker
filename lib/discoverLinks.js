...
// resolveTaxonScope, the run-bookkeeping helpers, the candidate-source helpers in lib/utils.js) is
// imported, not duplicated.
import { DiscoveryError, resolveIucn, resolveTaxonScope, runProgress, runFinishFields } from './discover.js';
import {
 ...
 *   log?: {warn: Function},
 *   sparqlFn?: (query: string) => Promise<import('./utils.js').SparqlBindingRow[]>,
 * }} opts
...
export async function discoverLinks({
    ...
    onProgress: report = () => {}, signal, candidateSource, log = console,
    // sparqlFn's default closes over `log` (destructured just above) — the two direct sparqlFn(...)
    // call sites below, and fetchWdAncestorChains, all get it threaded through with no further
    // change; a caller overriding sparqlFn gets whatever behaviour it wants instead.
    sparqlFn = (q) => sparql(q, 3, log),
}) {
    ...
    try {
        ...
        const source = candidateSource ?? (iucn
            ? fetchWdLinksByIucn(iucn.qid, { log })
            : fetchWdTaxaByNames(shuffle(taxaDb.allNames(), seed), { log }));
