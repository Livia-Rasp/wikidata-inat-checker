 ...
 * enough on its own. */
function isAutoEligible(evidence) {
...
WHERE {
  ...
  { ?item1 wdt:P13177 ?item2 } UNION { ?item2 wdt:P13177 ?item1 }
            ...
            for (const c of conflicts) {
                const evidence = compareAncestorTrees(
                    wdChains.get(c.qid) ?? [], taxaDb.getAncestors(c.inatId));
                store.upsertTaxon({
        ...
        if (scopedAmbiguous.length > 0) {
            ...
            for (const item of scopedAmbiguous) {
                ...
                const candidates = item.candidates.map(cand => {
                    ...
                    const evidence = compareAncestorTrees(wdChain, inatChainCache.get(cand.inatId));
                    return { inatId: cand.inatId, rank: cand.rank, evidence, score: null, scoredBy: null };
                });
                store.upsertTaxon({ qid: item.qid, taxonName: item.taxonName, iucn: item.iucn });
                store.recordFinding({ qid: item.qid, kind: KIND, status: 'ambiguous', payload: { candidates } });
                result.ambiguous++;
            }
        }
    } catch (err) {
