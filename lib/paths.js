...
// Where the tools write. Four kinds of artifact, kept apart on purpose:
...
// backups/ exists *because* data/ isn't safe to delete; deleting backups/ itself only costs you
// the snapshots, never the live database. All four are gitignored. (The large iNat taxa SQLite
// index lives separately under ~/.cache/wikidata-inat-checker/; it is derived and gets dropped and
// rebuilt, so it must never be confused with data/.) Paths are relative to the working directory —
// the repo root, where the tools are run.
