 ...
 * @param {{lat: number, lng: number, radius: number}} area
 * @param {{inatLimiter?: () => Promise<void>, getJsonFn?: (url: string) => Promise<any>,
 *           candidatesFn?: (ids: string[], opts?: {log?: {warn: Function}}) => AsyncIterable<any>,
 *           species?: Map<string, {taxonName: string, commonName: string, count: number}>,
 *           log?: {warn: Function}}} [opts]
 */
export async function* fetchAreaCandidates(area, opts = {}) {
    const { candidatesFn = fetchWdTaxaByInatIds, species: givenSpecies, log = console } = opts;
    const species = givenSpecies ?? await fetchAreaSpecies(area, opts);
    for await (const row of candidatesFn([...species.keys()], { log })) {
