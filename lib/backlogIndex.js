...
export function createBacklogIndex({ store, taxaDb, kind = 'image', status = 'open' }) {
    ...
    /**
     * Re-read every time. An earlier version cached the row list and invalidated it when a
     * discovery run finished, which was wrong in the way that matters here: **a skip or a confirm
     * changes the backlog without any run finishing**, so the search page kept offering work that
     * had just been settled — the same defect as a cached worklist, which is why every API response
     * carries `cache-control: no-store`.
     *
     * It is also not the expensive part. The scan is one indexed query over the findings table
     * (~1 ms for a backlog of 153); what cost half a second was clade membership, and that is what
     * `ancestorCache` keeps — keyed by taxon, so it survives this being called afresh.
     */
    function currentRows() {
        return store.listFindings({ kind, status });
    }
