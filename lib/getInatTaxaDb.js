...
export function createTaxaAccessor(db) {
    ...
    return {
        ...
        // Higher taxa first, then everything else. Someone typing a few letters is nearly always
        // reaching for a clade, and plain alphabetical order answers 'Orchi' with ten Orchidantha
        // species and buries Orchidaceae. Within each group, shallowest first — depth is the
        // ancestry path length, not a hardcoded rank vocabulary, so iNat can invent ranks freely.
        suggest(prefix, limit = 10) {
            const upper = prefixUpperBound(prefix);
            if (upper === undefined) return [];
            const depth = (r) => (r.ancestry ? String(r.ancestry).split('/').length : 0);
            const rank = (rows) => rows
                .map(r => ({ ...record(r), _depth: depth(r) }))
                .sort((a, b) => a._depth - b._depth || a.name.localeCompare(b.name));

            const seen = new Set();
            const out = [];
            for (const r of [...rank(stmtPrefixUni.all(prefix, upper, limit)),
