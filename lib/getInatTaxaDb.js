...
export function createTaxaAccessor(db) {
    ...
    return {
        get(name) {
            ...
            return { inatId: String(rows[0].taxon_id), rank: String(rows[0].rank) };
        },
        getAll(name) {
            return stmtAllByName.all(name).map(r => ({ inatId: String(r.taxon_id), rank: String(r.rank) }));
        },
        getAncestors(taxonId) {
            ...
            return String(row.ancestry).split('/').filter(Boolean)
                ...
                .map(a => ({ name: String(a.name), rank: String(a.rank) }));
        },
        allNames() {
            return stmtAllNames.all().map(r => String(r.name));
        },
        allInatIds() {
            return stmtAllIds.all().map(r => String(r.taxon_id));
        },
        descendantInatIds(taxonId) {
            ...
            return stmtDesc.all(
                `${taxonId}`, `${taxonId}/%`, `%/${taxonId}`, `%/${taxonId}/%`).map(r => String(r.taxon_id));
        },
...
export class TaxaIndexUnavailable extends AppError {
