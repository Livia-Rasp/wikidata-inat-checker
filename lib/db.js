...
/**
 * @typedef {{ id: number, qid: string, status: string, wdUri: string, inatTaxonId: string|null, taxonName: string|null, iucn: string|null, wikitext: string|null }} FindingRow
 */
...
export function createFindingsStore(db) {
    ...
    const stmtList = db.prepare(`
        SELECT f.id, f.qid, f.status, t.inat_id, t.taxon_name, t.iucn, f.payload
        FROM findings f
    ...
    return {
         ...
         * default: the HTML report and taxa.json render the *whole* backlog off this, so a default
         * page size here would silently truncate them. Callers that serve HTTP cap it themselves.
         */
        listFindings,
