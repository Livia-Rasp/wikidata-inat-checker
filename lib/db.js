...
/**
 * Statuses that mean "settled" — discovery must never resurface these. `ambiguous` and `conflict`
 * (kind=link) are settled in the sense that matters here: they already have a review row waiting,
 * so re-discovering them would just redo the SPARQL work and overwrite it with the same answer.
 */
export const STICKY_STATUSES = ['open', 'done', 'skipped', 'fixed_upstream', 'gone', 'ambiguous', 'conflict'];
 ...
 * `no_match` (kind=link) is the same idea — a name absent from the iNat index today may exist
 * there tomorrow.
 */
export const NEGATIVE_STATUSES = ['no_photos', 'no_draft', 'no_match'];
...
const MIGRATIONS = [
    ...
    // v5 (slice 8b): a skip used to be one global, sticky flag on the finding itself — right for a
    // single operator, wrong the moment a second tester's Skip silently hides a taxon from everyone
    // else forever. `findings.status` keeps its exact existing meaning (discovery/CLI/reports read
    // nothing new); it only becomes 'skipped' once every row in `clients` has skipped, or one skip
    // is marked `global` ("this will never have a usable photo", not just "not my area"). See
    // docs/findings-db-roadmap.md's "Known: skipped does not survive more than one user" section.
    function v5(db) {
        db.exec(`
            ...
            CREATE TABLE skips (
                ...
                PRIMARY KEY (qid, kind, client_id),
                FOREIGN KEY (qid, kind) REFERENCES findings(qid, kind)
            ) STRICT;
        `);
    },

    // v6 (slice 10): the discovery budget's own state. On-demand discovery (POST /discover, GET
    // /discover/area) used to be gated by a loopback-peer check, which a published Docker port can
    // never satisfy — replaced by an hourly-refilling token bucket per route, bounding cost instead
    // of checking identity. Store-backed rather than in-memory like the rate limiter: this
    // container is redeployed by Watchtower on Renovate's ordinary cadence, and a daily/hourly
    // budget must survive that the way a per-minute counter doesn't need to. See
    // docs/threat-model.md's "Discovery budget" section.
    function v6(db) {
        db.exec(`
            CREATE TABLE discover_budget (
...
// STRICT tables declare these columns TEXT, so this only names what the schema already
// guarantees — node:sqlite's SQLOutputValue union is otherwise honestly conservative about SQL's
// dynamic typing. Guards null first: String(null) is the string "null", not null.
/** @param {import('node:sqlite').SQLOutputValue} v @returns {string|null} */
...
export function createFindingsStore(db) {
    ...
    const stmtSetStatus = db.prepare(`
        UPDATE findings SET status = ?, resolved_at = ?, resolution = ?
        WHERE qid = ? AND kind = ?`);
    const stmtReopen = db.prepare(`
        UPDATE findings SET status = ?, resolved_at = NULL, resolution = NULL
        WHERE qid = ? AND kind = ?`);
    const stmtFindingStatus = db.prepare('SELECT status FROM findings WHERE qid = ? AND kind = ?');

    // Per-client skip tracking (slice 8b). registerClient is called on every skip/unskip so
    // "every known client" reflects everyone who has ever taken a skip-related action, not just
    // people who have actually skipped something.
    const stmtRegisterClient = db.prepare(
        'INSERT OR IGNORE INTO clients (id, first_seen) VALUES (?, ?)');
    const stmtClientCount = db.prepare('SELECT COUNT(*) AS n FROM clients');
    // prior_status is deliberately NOT overwritten on conflict: a client re-skipping the same
    // finding must not lose the status it originally saw in favour of a later, possibly-already-
    // 'skipped' one.
    const stmtUpsertSkip = db.prepare(`
        INSERT INTO skips (qid, kind, client_id, reason, global, prior_status, skipped_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(qid, kind, client_id) DO UPDATE SET
            reason     = excluded.reason,
            global     = excluded.global,
            skipped_at = excluded.skipped_at`);
    const stmtSkipRow = db.prepare(
        'SELECT prior_status, global FROM skips WHERE qid = ? AND kind = ? AND client_id = ?');
    const stmtDeleteSkip = db.prepare(
        'DELETE FROM skips WHERE qid = ? AND kind = ? AND client_id = ?');
    const stmtSkipCoverage = db.prepare(
        'SELECT COUNT(*) AS n, COALESCE(MAX(global), 0) AS any_global FROM skips WHERE qid = ? AND kind = ?');
    ...
    // The client_id = ? clause is always present, even with no clientId to exclude: an empty
    // string never matches a real (non-empty) generated client id, so passing '' is a no-op
    // exclusion rather than needing a second prepared statement for the no-client case.
    const stmtList = db.prepare(`
        ...
        WHERE f.kind = ? AND f.status = ?
          AND NOT EXISTS (
              SELECT 1 FROM skips s WHERE s.qid = f.qid AND s.kind = f.kind AND s.client_id = ?)
    ...
    // Backups (slice 9, tools/backup.mjs). VACUUM INTO takes a read lock and produces a consistent
    // snapshot regardless of journal mode — safe to run against a database another process (the
    // server, a checker) is actively writing to, which is the whole point of running it on a timer
    // rather than requiring the app to be stopped first.
    const stmtVacuumInto = db.prepare('VACUUM INTO ?');

    // Discovery budget (slice 10): one row per bucket ('discover', 'discover_area'), lazily
    // created at full capacity on first touch — same "nothing pre-seeded" pattern as everything
    // else in this store.
    const stmtGetBudget = db.prepare('SELECT tokens, updated_at FROM discover_budget WHERE bucket = ?');
    const stmtUpsertBudget = db.prepare(`
        INSERT INTO discover_budget (bucket, tokens, updated_at) VALUES (?, ?, ?)
        ON CONFLICT(bucket) DO UPDATE SET tokens = excluded.tokens, updated_at = excluded.updated_at`);

    /**
     * @param {{kind: string, status?: string, limit?: number|null, offset?: number, clientId?: string|null}} q
     * @returns {FindingRow[]}
     */
    const listFindings = ({ kind, status = 'open', limit = null, offset = 0, clientId = null }) =>
        stmtList.all(kind, status, clientId ?? '', limit ?? -1, offset).map(r => ({
            id: Number(r.id),
            qid: String(r.qid),
            kind: String(r.kind),
            status: String(r.status),
            wdUri: `http://www.wikidata.org/entity/${r.qid}`,
            inatTaxonId: strOrNull(r.inat_id),
            taxonName: strOrNull(r.taxon_name),
            iucn: strOrNull(r.iucn),
            ...
            wikitext: r.payload ? JSON.parse(String(r.payload)).wikitext ?? null : null,
    ...
    return {
        ...
        skipQids(kind, recheckAfterDays = DEFAULT_RECHECK_DAYS) {
            ...
            return new Set(rows.map(r => String(r.qid)));
        },
        ...
        /** Note that `clientId` has taken some skip-related action, so it counts as a "known
         *  client" for coverage even before it has skipped anything itself.
         *  @param {string} clientId */
        registerClient(clientId) {
            stmtRegisterClient.run(clientId, new Date().toISOString());
        },

        /**
         * Record `clientId`'s decision to pass over (qid, kind). `findings.status` only actually
         * becomes 'skipped' — sticky, so discovery never resurfaces it — once every known client
         * has skipped it too, or this one skip is marked `global` ("this will never have a usable
         * photo", not "not my area"). Otherwise the finding stays exactly as it was; only this
         * client's own worklist view (via `listFindings`'s `clientId` exclusion) hides it.
         * @param {string} qid @param {string} kind @param {string} clientId
         * @param {{reason?: string, global?: boolean}} [opts]
         */
        recordSkip(qid, kind, clientId, { reason, global = false } = {}) {
            this.registerClient(clientId);
            const finding = stmtFindingStatus.get(qid, kind);
            const priorStatus = finding ? String(finding.status) : 'open';
            const now = new Date().toISOString();
            stmtUpsertSkip.run(qid, kind, clientId, reason ?? null, global ? 1 : 0, priorStatus, now);

            if (global) {
                stmtSetStatus.run(
                    'skipped', now, JSON.stringify({ reason: reason ?? null, global: true }), qid, kind);
                return;
            }
            const { n, any_global } = stmtSkipCoverage.get(qid, kind);
            const { n: clientTotal } = stmtClientCount.get();
            if (any_global || Number(n) >= Number(clientTotal)) {
                stmtSetStatus.run('skipped', now, JSON.stringify({ reason: reason ?? null }), qid, kind);
            }
        },

        /**
         * Reverse `clientId`'s own skip. A **legacy** skip — one recorded before per-client
         * tracking existed, with status 'skipped' but no row in `skips` at all — has no per-client
         * attribution to defer to, so any client may undo it outright.
         * @param {string} qid @param {string} kind @param {string} clientId
         */
        recordUnskip(qid, kind, clientId) {
            const own = stmtSkipRow.get(qid, kind, clientId);
            stmtDeleteSkip.run(qid, kind, clientId);

            const finding = stmtFindingStatus.get(qid, kind);
            if (!finding || finding.status !== 'skipped') return; // nothing settled to reopen

            if (!own) {
                stmtReopen.run('open', qid, kind);
                return;
            }
            const { n, any_global } = stmtSkipCoverage.get(qid, kind);
            const { n: clientTotal } = stmtClientCount.get();
            if (!any_global && Number(n) < Number(clientTotal)) {
                stmtReopen.run(String(own.prior_status), qid, kind);
            }
        },
        ...
        getFinding(id) {
            ...
            return r ? {
                id: Number(r.id), qid: String(r.qid), kind: String(r.kind), status: String(r.status),
                payload: r.payload ? JSON.parse(String(r.payload)) : null,
            } : undefined;
        },
        ...
        /** @returns {Record<string, {destFile: string, taxonName: string|null, findingId: number|null}>} qid → pick */
        p18Picks() {
        ...
        latestRun(tool, { triggeredBy } = {}) {
            ...
            return {
                id: Number(r.id),
                tool: r.tool,
                scope: r.scope ? JSON.parse(String(r.scope)) : null,
                state: r.state,
                error: r.error ?? null,
        ...
        quietHoursOfDay({ lookbackDays, quietHoursCount }) {
            ...
            const averages = new Map(
                stmtHourAverages.all(since).map(r => [Number(r.hour), Number(r.avg_count)]));
        ...
        /**
         * Write a consistent snapshot of the whole database to `destPath` (which must not already
         * exist — SQLite refuses to overwrite a file with VACUUM INTO). The parent directory is
         * the caller's responsibility, same as `openFindingsDb`.
         * @param {string} destPath
         */
        vacuumInto(destPath) {
            stmtVacuumInto.run(destPath);
        },

        /**
         * Refill `bucket` for elapsed wall-clock time, then draw one token if at least one is
         * available. BEGIN IMMEDIATE — same discipline migrate() uses — so two near-simultaneous
         * requests cannot both see the same last token and both be admitted. A bucket touched for
         * the first time starts full: `updated_at` reads as "now", so no time has elapsed to
         * refill.
         * @param {string} bucket @param {{capacity: number, refillPerHour: number}} cfg
         * @returns {{admitted: boolean, tokensRemaining: number}}
         */
        drawDiscoverBudget(bucket, { capacity, refillPerHour }) {
            db.exec('BEGIN IMMEDIATE');
            try {
                const nowMs = Date.now();
                const row = stmtGetBudget.get(bucket);
                const prevTokens = row ? Number(row.tokens) : capacity;
                const prevAtMs = row ? Date.parse(String(row.updated_at)) : nowMs;
                const elapsedHours = Math.max(0, (nowMs - prevAtMs) / 3_600_000);
                const refilled = Math.min(capacity, prevTokens + elapsedHours * refillPerHour);
                const admitted = refilled >= 1;
                const tokensRemaining = admitted ? refilled - 1 : refilled;
                stmtUpsertBudget.run(bucket, tokensRemaining, new Date(nowMs).toISOString());
                db.exec('COMMIT');
                return { admitted, tokensRemaining };
            } catch (err) {
                db.exec('ROLLBACK');
                throw err;
            }
        ...
        /**
         * Give one token back to `bucket`, capped at `capacity` — for a draw whose jobs.start()
         * lost the single-flight race, so the run it was drawn for never actually happened.
         * @param {string} bucket @param {number} capacity
         */
        refundDiscoverBudget(bucket, capacity) {
            db.exec('BEGIN IMMEDIATE');
            try {
                const row = stmtGetBudget.get(bucket);
                const prevTokens = row ? Number(row.tokens) : capacity;
                const tokens = Math.min(capacity, prevTokens + 1);
                stmtUpsertBudget.run(
                    bucket, tokens, row ? String(row.updated_at) : new Date().toISOString());
                db.exec('COMMIT');
            } catch (err) {
                db.exec('ROLLBACK');
                throw err;
        ...
        /**
         * Tokens `bucket` would have available right now, without drawing — the scheduler's
         * bonus-draw check needs to ask "how much is left" before deciding whether there is
         * enough unused capacity to take a bonus run. Computes the same refill drawDiscoverBudget
         * does, but never writes, so a status check costs nothing and never perturbs `updated_at`.
         * @param {string} bucket @param {{capacity: number, refillPerHour: number}} cfg
         * @returns {{tokensRemaining: number}}
         */
        discoverBudgetStatus(bucket, { capacity, refillPerHour }) {
            const row = stmtGetBudget.get(bucket);
            if (!row) return { tokensRemaining: capacity };
            const elapsedHours = Math.max(0, (Date.now() - Date.parse(String(row.updated_at))) / 3_600_000);
            const tokensRemaining = Math.min(capacity, Number(row.tokens) + elapsedHours * refillPerHour);
