...
/**
 * @typedef {{ id: number, qid: string, kind: string, status: string, wdUri: string, inatTaxonId: string|null, taxonName: string|null, iucn: string|null, wikitext: string|null }} FindingRow
 */
...
export function createFindingsStore(db) {
    ...
    const stmtFindingById = db.prepare(
        'SELECT id, qid, kind, status FROM findings WHERE id = ?');
    ...
    const stmtPicks = db.prepare(
        'SELECT qid, dest_file, taxon_name FROM uploads WHERE is_p18 = 1 AND qid IS NOT NULL');
    ...
    const stmtReconcile = db.prepare(`
        ...
        WHERE state = 'running'`);
    const runColumns = 'id, tool, scope, state, error, started_at, finished_at, n_scanned, n_found';
    ...
    return {
         ...
         * way to tell an unknown id from a successful no-op.
         * @param {number} id
         * @returns {{id: number, qid: string, kind: string, status: string}|undefined}
         */
        getFinding(id) {
            const r = stmtFindingById.get(id);
            return r ? { id: Number(r.id), qid: r.qid, kind: r.kind, status: r.status } : undefined;
        },
         ...
         * user's own claim until slice 10 does the upload itself.
        ...
        p18Picks() {
            return Object.fromEntries(stmtPicks.all().map(
                r => [r.qid, { destFile: r.dest_file, taxonName: r.taxon_name ?? null }]));
        },
        ...
        /** The most recent run, whatever became of it. @param {string} [tool] */
        latestRun(tool) {
            const r = tool ? stmtLatestRunByTool.get(tool) : stmtLatestRunAny.get();
            if (!r) return null;
            return {
                id: Number(r.id),
                tool: r.tool,
