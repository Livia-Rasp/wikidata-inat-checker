# iNat → Commons upload step (research & design)

Research notes and design options for adding an **assisted upload** step to the image
pipeline: take an image-less Wikidata taxon, surface compatibly-licensed iNaturalist
photos, and let the user open Commons' upload form **pre-filled** with all the right
parameters — one click away from publishing, but never fully automated.

Status: **implemented** (initial version). Built as an **interactive local web app**
(Option B), **inside this repo** in `web/`, with a planned spin-out into its own repo once
`wikidata-inat-checker` goes public and the web service is published. This documents how
the existing [`lubianat/inat2wiki`](https://github.com/lubianat/inat2wiki) family works,
the technical constraints we verified, and the chosen design.
...
— but a defensive check (only emit a prefill link when the photo's host is one of the two
allowlisted ones; otherwise fall back to a manual upload link) is cheap insurance.
...
with the core here, or the core graduates into a published package. The relevant durable
details from this doc should migrate into `dev.md` / `images.md` then.
...
1. **Data export (CLI, root).** Finding image-less taxa needs the SQLite index
   ...
   load. Extend `checkImages.js` (or a small sibling) to **export the taxon list as JSON**
   (e.g. `web/data/taxa.json`, gitignored) instead of/alongside `drafts.html`.
...
a local JSON over `file://`. Any of `python -m http.server` / `npx serve` / a ~10-line Node
static-server script suffices — this is *not* an application backend.
