...
- **`data/`** — `findings.db` only, and unlike the other two it is **not safe to delete**: it is the accumulated backlog and the record of what has been worked through, and nothing can reconstruct it. See [Findings database](#findings-database-libdbjs) below.
