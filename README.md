...
iNat queries are batched via the `/v1/observations/species_counts` endpoint (up to 50 taxa per request), keeping a full ~5000-taxon scan to about a minute while staying within iNat's recommended ~1 request/second rate.
...
This runs the SPARQL query and the iNat check in a single pass. Outputs:
